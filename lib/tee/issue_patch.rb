...
module TEE
  module IssuePatch
    ...
    module ClassMethods
      ...
      # Este método se usará para ver el tiempo total en el último estado en el que se encuentra 
      # en el listado de las peticiones como una columna opcional disponible
      # al igual que para que se mostrado el tiempo total en formato PDF y SCV.
      def get_total_time_last_status(issue_id, project)
        ...
        return ((Issue.get_hours(@last_interval_time) * 10).floor / 10.0).to_s
      end
    ...
    module InstanceMethods
      ...
      def get_start_statuses_ans(issue)
        start_statuses = {}
        all_statuses = IssueStatus.all.map{|s| { :id => s.id, :name => s.name}}
        ...
        return start_statuses
      end

      def get_pause_statuses_ans(issue)
        pause_statuses = {}
        all_statuses = IssueStatus.all.map{|s| { :id => s.id, :name => s.name}}
